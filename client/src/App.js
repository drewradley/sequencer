    ...
    componentDidMount() {
      ...
      handleSubmit = async e => {
        ...
        const response = await fetch('/api/world', {
          ...
          body: JSON.stringify({ post: JSON.stringify(seqs) })
        });
        const body = await response.text();
        // this.setState({ email: email });
        this.setState({ responseToPost: body });
        alert("Sequence has been saved")
        ...
        for(let i=0;i<seqs.length;i++)
        {
            ...
            if(seqs[i].course.series>0)
            {
                for(let v=0;v<curTermCoursesLeft.length;v++)
                {
                    ...
                    {
                        alert("You have taken "+seqs[i].course.name +": "+seqs[i].course.title +".\nIt is recommended that you take "+curTermCoursesLeft[v].name+": "+curTermCoursesLeft[v].title)
                    }
                }
                for(let v=0;v<curTermCoursesRight.length;v++)
                {
                    ...
                    {
                        alert("You have taken "+seqs[i].course.name +": "+seqs[i].course.title +".\nIt is recommended that you take "+curTermCoursesRight[v].name+": "+curTermCoursesRight[v].title)
                    }
                }
            }
        }
    ...
    render(){
        ...
        <p></p>
             {unitsLeft>0 ?(""):(<div>
                        ...
                        /> */}
                        <button type="submit" className="remove btn btn-warning">Save Sequence</button>
                        </form>
                        <p></p>
